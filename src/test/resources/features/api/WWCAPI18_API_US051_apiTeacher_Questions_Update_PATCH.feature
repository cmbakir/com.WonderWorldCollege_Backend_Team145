Feature: As a teacher (teacher), I want to be able to update the question information registered in the system via API connection.

  Scenario Outline: Verify that a PATCH request to `/apiteacher/questionUpdate` with valid authorization and correct data
  ...
  returns a 200 status code, a "Success" message, and that the `updateId` in the response matches the `id` in the request body.
    ...
    * The api user verifies that the updateid information in the response body is the same as the id information in the request body.
    ...
    Examples:
      ...
      |   id | subject_id | question_type | level    | class_id | section_id | class_section_id | question  | opt_a   | opt_b   | opt_c   | opt_d   | opt_e     | correct | descriptive_word_limit |
      | 221  | 4          | true_false    | low        | 11     | 1          |null              | 1+1=2     |         |         |         |         |           |   true  | 0                      |



  Scenario Outline: Verify that a PATCH request to `/apiteacher/questionUpdate` with valid authorization and invalid `id`
  ...
  message in the response body.
    ...
    * The api user verifies that the "message" information in the response body is "Wrong information or missing information. Please check your input data and id number.".
    ...
    Examples:
      |   id   | subject_id | question_type | level    | class_id | section_id | class_section_id | question  | opt_a | opt_b   | opt_c   | opt_d   | opt_e     | correct | descriptive_word_limit |
      | 12345  | 4          | true_false    | low      | 11       | 1          |null              | 1+1=2     |       |         |         |         |           |   true  | 0                      |


  Scenario Outline: Verify that a PATCH request to `/apiteacher/questionUpdate` without `id` and with valid authorization
  returns a 203 status code and a "Wrong information or missing information. Please check your input data and id number."
  message in the response body.
  ...
  Scenario: Verify that a PATCH request to `/apiteacher/questionUpdate` without valid authorization and data returns a
  ...
  Scenario Outline: Verify that a PATCH request to `/apiteacher/questionUpdate` with invalid authorization and data (id, subject_id, question_type, level, class_id, section_id, class_section_id, question, opt_a, opt_b, opt_c, opt_d, opt_e, correct, descriptive_word_limit)
  ...
  Scenario Outline: Verify that the questions record updated via API is successfully updated by sending a POST request to
  `/apiteacher/questionDetailsById` with the `updateId` returned in the response body.
    ...
    * The api user verifies question  as "question update"
